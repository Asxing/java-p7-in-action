...
/**
 * 简单timer： 1) 单线程检测 2) 将任务封装到JavaBean 3) 时间到自动执行
 *
 * @author HoldDie
 */
public class Timer {
    ...
    /**
     * 检测执行时间是否到达： 1.从队列拿到任务的时候，时间到了，直接执行 2.时间没到，阻塞直到时间到达 3.阻塞期间，被唤醒，说明有新任务到达，此时检测时间是否到了，没到放回队列里面
    ...
    public static void main(String[] args) {
        ...
        timer.scheduleAtTime(
                5000 + System.currentTimeMillis(),
                () -> {
                    System.out.println("run at five seconds later.");
                });
        try {
            Thread.sleep(10000);
        } catch (InterruptedException e) {

...
public class StreamDetailTest {
        ...
        orders.stream()
                .collect(
                        groupingBy(
                                Order::getCustomerName,
                                collectingAndThen(
                                        maxBy(comparingDouble(Order::getTotalPrice)),
                                        Optional::get)))
                .forEach(
                        (k, v) ->
                                System.out.println(
                                        k + "#" + v.getTotalPrice() + "@" + v.getPlacedAt()));
    ...
    public void distinct() {
        ...
        System.out.println(
                orders.stream().map(order -> order.getCustomerName()).collect(joining(",")));
    ...
    public void collect() {
        ...
        System.out.println(
                orders.stream().limit(2).collect(toCollection(LinkedList::new)).getClass());
    ...
    public void customCollector() // 自定义收集器
        ...
        // 最受欢迎收集器
        assertThat(
                Stream.of(1, 1, 2, 2, 2, 3, 4, 5, 5).collect(new MostPopularCollector<>()).get(),
                is(2));
        assertThat(
                Stream.of('a', 'b', 'c', 'c', 'c', 'd').collect(new MostPopularCollector<>()).get(),
                is('c'));
        assertThat(
                Stream.concat(
                                Stream.concat(
                                ...
                                Stream.of(2))
                        .parallel()
                        .collect(new MostPopularCollector<>())
                        .get(),
                is(2));
    }
}
